...
int hermon_write_conf_tlv ( struct hermon *hermon,
				struct hermon_tlv_header *tlv_header )
{
	return hermon_access_tlv(hermon, tlv_header, HERMON_TLV_ACCESS_WRITE);
}

int hermon_invalidate_conf_tlv ( struct hermon *hermon,
				struct hermon_tlv_header *tlv_header )
{
	return hermon_access_tlv(hermon, tlv_header, HERMON_TLV_ACCESS_INVALIDATE);
}

int hermon_get_tlv_version ( uint32_t tlv_type ) {
	int version = 0;
...
int hermon_flash_read_nic_boot_config ( struct hermon *hermon, struct hermon_port *port ) {
	union hermon_nic_boot_conf *conf = & ( port->port_nv_conf.nic.boot_conf );
	struct hermon_tlv_header tlv;
	int rc;

	memset ( &tlv, 0, sizeof ( tlv ) );
	tlv.type	= BOOT_SETTINGS_TYPE;
	tlv.type_mod	= port->ibdev->port;
	tlv.length	= sizeof ( *conf );
	tlv.data	= ( void* ) conf;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) ) {
		DBGC ( hermon, "Failed to read the boot configurations from the flash (rc = %d)\n", rc );
		conf->en_option_rom	= port->defaults.boot_option_rom_en;
		conf->en_vlan		= port->defaults.boot_vlan_en;
		conf->boot_retry_count	= port->defaults.boot_retry_count;
		conf->legacy_boot_prot	= port->defaults.boot_protocol;
		conf->vlan_id		= port->defaults.boot_vlan;
	}

	return rc;
}

int hermon_flash_read_nic_wol_config ( struct hermon *hermon, struct hermon_port *port ) {
	struct hermon_tlv_header tlv;
	union hermon_wol_conf *conf = & ( port->port_nv_conf.nic.wol_conf );
	int rc;

	/* Read WOL configuration */
	memset ( &tlv, 0, sizeof ( tlv ) );
	tlv.type	= WAKE_ON_LAN_TYPE;
	tlv.type_mod	= port->ibdev->port;
	tlv.length	= sizeof ( *conf );
	tlv.data	= ( void* ) conf;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) ) {
		conf->en_wol_magic = port->defaults.en_wol_magic;
		DBGC ( hermon, "Failed to read the WOL configuration from the flash (rc = %d)\n", rc );
	}

	return rc;
}

static void hermon_iscsi_init_dhcp_no_tlv ( struct hermon *hermon, unsigned int port,
		union hermon_iscsi_init_dhcp_conf *init_dhcp_conf ) {
	struct hermon_iscsi_initiator_params *init_params =
			& ( hermon->port[port].port_nv_conf.iscsi.initiator_params );
	struct hermon_iscsi_target_params *tgt_params =
			& ( hermon->port[port].port_nv_conf.iscsi.first_tgt_params );
...
static void hermon_flash_read_iscsi_init_dhcp ( struct hermon *hermon, unsigned int port,
		union hermon_iscsi_init_dhcp_conf *init_dhcp_conf ) {
	struct hermon_tlv_header tlv;
	int rc;

	memset ( &tlv, 0, sizeof ( tlv ) );
	tlv.type        = ISCSI_INITIATOR_DHCP_CONF_TYPE;
	tlv.type_mod    = port + 1;
	tlv.length      = sizeof( *init_dhcp_conf );
	tlv.data        = (void*) init_dhcp_conf;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 ) {
		DBGC ( hermon, "Failed to read iSCSI initiator DHCP settings (rc = %d)."
				" Using default behavior.\n", rc );
		hermon_iscsi_init_dhcp_no_tlv ( hermon, port, init_dhcp_conf );
	}
}

static void hermon_flash_read_iscsi_general ( struct hermon *hermon, unsigned int port,
		union hermon_iscsi_general *gen_conf ) {
	struct hermon_port_conf_defaults *defaults = & ( hermon->port[port].defaults );
	struct hermon_iscsi_target_params *target = & ( hermon->port[port].port_nv_conf.iscsi.first_tgt_params );
	struct hermon_iscsi_initiator_params *initiator = & ( hermon->port[port].port_nv_conf.iscsi.initiator_params );
	struct hermon_tlv_header tlv;
	int rc;
	...
	tlv.type_mod    = port + 1;
	tlv.length      = sizeof( *gen_conf );
	tlv.data        = (void*) gen_conf;

	rc = hermon_read_conf_tlv ( hermon, &tlv );
	if ( rc || ( !rc && ( tlv.version == 0 ) ) ) {
		if ( rc ) {
			DBGC ( hermon, "Failed to read iSCSI general settings (rc = %d)\n", rc );
			gen_conf->chap_mutual_auth_en	= defaults->iscsi_chap_mutual_auth_en;
			gen_conf->chap_auth_en			= defaults->iscsi_chap_auth_en;
		} else {
			DBGC ( hermon, "Found iSCSI general settings TLV version 0 \n");
...
static void hermon_flash_read_iscsi_initiator ( struct hermon *hermon, unsigned int port,
					struct hermon_iscsi_initiator_params *initiator_params ) {
	struct hermon_tlv_header tlv;
	int rc;
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( initiator_params->ipv4_addr );
	tlv.data	= initiator_params->ipv4_addr;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 ) {
		DBGC ( hermon, "Failed to read iSCSI Initiator 'IPv4' setting (rc = %d)\n", rc );
	} else {
		initiator_params->ipv4_addr[tlv.length] = '\0';
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( initiator_params->subnet );
	tlv.data        = initiator_params->subnet;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'Subnet' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( initiator_params->gateway );
	tlv.data        = initiator_params->gateway;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'Gateway' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length      = sizeof ( initiator_params->primary_dns );
	tlv.data        = initiator_params->primary_dns;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'Primary DNS' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length      = sizeof ( initiator_params->name );
	tlv.data        = initiator_params->name;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'Name' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( initiator_params->chap_id );
	tlv.data        = initiator_params->chap_id;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'CHAP ID' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length      = sizeof ( initiator_params->chap_pass );
	tlv.data        = initiator_params->chap_pass;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI Initiator 'CHAP Password' setting (rc = %d)\n", rc );
	else
		initiator_params->valid_mask |= ISCSI_INITIATOR_CHAP_PASS_MASK;

}

static void hermon_flash_read_iscsi_first_tgt ( struct hermon *hermon, unsigned int port,
					struct hermon_iscsi_target_params *first_tgt_params ) {
	struct hermon_tlv_header tlv;
	int rc;
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->connect );
	tlv.data	= first_tgt_params->connect;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 ) {
		DBGC ( hermon, "Failed to read iSCSI first target 'Connect' setting (rc = %d)\n", rc );
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->ip_addr );
	tlv.data	= first_tgt_params->ip_addr;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'IP' setting (rc = %d)\n", rc );
	else {
		first_tgt_params->ip_addr[tlv.length] = '\0';
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->tcp_port );
	tlv.data	= first_tgt_params->tcp_port;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'TCP port' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->lun );
	tlv.data	= first_tgt_params->lun;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'LUN' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->name );
	tlv.data	= first_tgt_params->name;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'Name' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->chap_id );
	tlv.data	= first_tgt_params->chap_id;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'CHAP ID' setting (rc = %d)\n", rc );
	else
	...
	tlv.type_mod	= port + 1;
	tlv.length	= sizeof ( first_tgt_params->chap_pass );
	tlv.data	= first_tgt_params->chap_pass;

	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) != 0 )
		DBGC ( hermon, "Failed to read iSCSI first target 'CHAP Password' setting (rc = %d)\n", rc );
	else
		first_tgt_params->valid_mask |= ISCSI_TARGET_CHAP_PASS_MASK;

}

int hermon_flash_read_iscsi_config ( struct hermon *hermon, unsigned int port,
					struct hermon_iscsi_conf *iscsi_conf ) {
	/* Read Initiator settings */
	hermon_flash_read_iscsi_initiator ( hermon, port, & ( iscsi_conf->initiator_params ) );
	/* Read first target settings */
	hermon_flash_read_iscsi_first_tgt ( hermon, port, & ( iscsi_conf->first_tgt_params ) );
	/* Read General settings - Needs to be after the above two functions
	 * 'Connect' parameter from target
	 * 'IP' parameter from initiator
	 */
	hermon_flash_read_iscsi_init_dhcp ( hermon, port, & ( iscsi_conf->init_dhcp_conf ) );

	hermon_flash_read_iscsi_general ( hermon, port, & ( iscsi_conf->gen_conf ) );

	return 0;
}

int hermon_flash_read_fw_image_props ( struct hermon *hermon,
	...
	snprintf ( fw_image_props->family_fw_version, sizeof ( fw_image_props->family_fw_version ),
		"%d.%d.%d", MLX_GET ( &fw, fw_rev_major ), MLX_GET ( &fw, fw_rev_minor ),
		MLX_GET ( &fw, fw_rev_subminor ));

	/* Get Flexboot version */
#ifdef __BASE_BUILD_VERSION__
	/* If not compiled with version number - put the default verison */
	strcpy ( fw_image_props->flexboot_version, __BASE_BUILD_VERSION__ );
#else
	strcpy ( fw_image_props->flexboot_version, __BUILD_VERSION__ );
#endif
	return 0;
}

static int hermon_flash_read_general_settings ( struct hermon_port *port ) {
...
#define HERMON_STORE_SETTING( settings, setting, data )				\
	do {									\
		if ( ( rc = storef_setting ( settings, &setting, data ) ) ) 	\
			DBGC ( hermon, "Failed to store %s\n", setting.name );	\
	} while ( 0 )

#define HERMON_ISCSI_STORE_STR_TO_SETTING( setting, is_valid, mask, data )	\
	do {									\
		if ( is_valid & mask ) {					\
			HERMON_STORE_SETTING( settings, setting, data );	\
		}								\
	} while (0)

#define HERMON_ISCSI_STORE_IP_TO_SETTING( setting, is_valid, mask, data )		\
	do {										\
		if ( is_valid & mask ) {						\
...
static int hermon_set_iscsi_settings ( struct hermon_port *port,
				struct hermon_iscsi_conf *iscsi_conf ) {
	struct settings *settings;
	struct settings *main_settings;
	struct hermon_iscsi_initiator_params *initiator = &( iscsi_conf->initiator_params );
	struct hermon_iscsi_target_params *first_tgt = &( iscsi_conf->first_tgt_params );
	union hermon_iscsi_init_dhcp_conf *init_dhcp_conf = &( iscsi_conf->init_dhcp_conf );
	union hermon_iscsi_general *gen_conf = &( iscsi_conf->gen_conf );
	struct hermon *hermon =  ib_get_drvdata( port->ibdev );
	struct in_addr ip_addr;
	int rc;

	main_settings = & ( port->driver_settings.generic_settings.settings );
	/* Store general settings */
	rc = fetch_setting_origin ( main_settings, &ip_ver_setting, &settings );
        if ( rc || ( !rc && !settings ) ) {
                DBGC ( hermon, "Failed to find the iSCSI general settings block\n" );
                return rc;
        }
	/* Currently only IPv4 is supported */
	HERMON_STORE_SETTING( settings, ip_ver_setting, STR_IPV4 );
	HERMON_STORE_SETTING( settings, dhcp_ip_setting,
			( init_dhcp_conf->ipv4_dhcp_en ? STR_ENABLE : STR_DISABLE ) );
	HERMON_STORE_SETTING( settings, dhcp_iscsi_setting,
			( init_dhcp_conf->dhcp_iscsi_en ? STR_ENABLE : STR_DISABLE ) );
	HERMON_STORE_SETTING( settings, iscsi_chap_setting,
			( gen_conf->chap_auth_en ? STR_ENABLE : STR_DISABLE ) );
	HERMON_STORE_SETTING( settings, iscsi_mutual_chap_setting,
	...
	if ( rc || ( !rc && !settings ) ) {
		DBGC ( hermon, "Failed to find the iSCSI initiator's settings block\n" );
		return rc;
	}

	HERMON_ISCSI_STORE_IP_TO_SETTING( ipv4_add_setting, initiator->valid_mask,
		ISCSI_INITIATOR_IP_ADDR_MASK, initiator->ipv4_addr );
	HERMON_ISCSI_STORE_IP_TO_SETTING( subnet_mask_setting, initiator->valid_mask,
		ISCSI_INITIATOR_SUBNET_MASK, initiator->subnet );
	HERMON_ISCSI_STORE_IP_TO_SETTING( ipv4_gateway_setting, initiator->valid_mask,
		ISCSI_INITIATOR_GATEWAY_MASK, initiator->gateway );
	HERMON_ISCSI_STORE_IP_TO_SETTING( ipv4_dns_setting, initiator->valid_mask,
		ISCSI_INITIATOR_PRIME_DNS_MASK, initiator->primary_dns );
	HERMON_ISCSI_STORE_STR_TO_SETTING ( iscsi_init_name_setting, initiator->valid_mask,
		ISCSI_INITIATOR_NAME_MASK, initiator->name );
	HERMON_ISCSI_STORE_STR_TO_SETTING ( init_chapid_setting, initiator->valid_mask,
		ISCSI_INITIATOR_CHAP_ID_MASK, initiator->chap_id );
	HERMON_ISCSI_STORE_STR_TO_SETTING ( init_chapsec_setting, initiator->valid_mask,
	...
	HERMON_ISCSI_STORE_STR_TO_SETTING( connect_setting, first_tgt->valid_mask,
		ISCSI_TARGET_CONNECT_MASK, first_tgt->connect );
	HERMON_ISCSI_STORE_IP_TO_SETTING( target_ip_setting, first_tgt->valid_mask,
		ISCSI_TARGET_IP_ADDR_MASK, first_tgt->ip_addr );
	HERMON_ISCSI_STORE_STR_TO_SETTING( tcp_port_setting, first_tgt->valid_mask,
		ISCSI_TARGET_TCP_PORT_MASK, first_tgt->tcp_port );
	HERMON_ISCSI_STORE_STR_TO_SETTING( boot_lun_setting, first_tgt->valid_mask,
		ISCSI_TARGET_LUN_MASK, first_tgt->lun );
	HERMON_ISCSI_STORE_STR_TO_SETTING( iscsi_target_name_setting, first_tgt->valid_mask,
		ISCSI_TARGET_NAME_MASK, first_tgt->name );
	HERMON_ISCSI_STORE_STR_TO_SETTING( target_chapid_setting, first_tgt->valid_mask,
		ISCSI_TARGET_CHAP_ID_MASK, first_tgt->chap_id );
	HERMON_ISCSI_STORE_STR_TO_SETTING( target_chapsec_setting, first_tgt->valid_mask,
		ISCSI_TARGET_CHAP_PASS_MASK, first_tgt->chap_pass );

	return 0;
}

/*TODO: define the strings for the settings */
#define HERMON_MAX_STR_LEN_SETTING	255
#define HERMON_STORE_STR_SETTING( settings, setting, data )			\
	do {									\
		memset ( buf, 0, sizeof ( buf ) );				\
		snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%s", data );	\
		HERMON_STORE_SETTING( settings, setting, buf );			\
	} while ( 0 )

#define HERMON_STORE_INT_SETTING( settings, setting, data )                     \
	do {									\
		memset ( buf, 0, sizeof ( buf ) );				\
		snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%d", data );	\
		HERMON_STORE_SETTING( settings, setting, buf );			\
	} while ( 0 )

static int hermon_set_nic_settings ( struct hermon_port *port, struct hermon_nic_conf *nic_conf ) {
	struct hermon *hermon = ib_get_drvdata( port->ibdev );
	struct settings *menu_settings = & ( port->driver_settings.generic_settings.settings );
	union hermon_nic_boot_conf *conf = & ( nic_conf->boot_conf );
	struct settings *origin;
	char buf[HERMON_MAX_STR_LEN_SETTING];
	int rc;

	rc = fetch_setting_origin ( menu_settings, &boot_protocol_setting, &origin );
	if ( rc || ( !rc && !origin ) ) {
		DBGC ( hermon, "Failed to find NIC configuration settings block\n" );
		return rc;
	}
	...
	if ( conf->legacy_boot_prot == BOOT_PROTOCOL_PXE )
		snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%s", STR_PXE );
	else if (conf->legacy_boot_prot == BOOT_PROTOCOL_ISCSI )
		snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%s", STR_ISCSI );
	else
		snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%s", STR_NONE );

	HERMON_STORE_SETTING( origin, boot_protocol_setting, buf );
	HERMON_STORE_STR_SETTING( origin, virt_lan_setting, ( conf->en_vlan ? STR_ENABLE : STR_DISABLE ) );
	HERMON_STORE_INT_SETTING( origin, virt_id_setting, conf->vlan_id );
	HERMON_STORE_STR_SETTING( origin, opt_rom_setting, ( conf->en_option_rom ? STR_ENABLE : STR_DISABLE ) );

	/* Find boot retry string to save according to boot retry number */
	snprintf ( buf, HERMON_MAX_STR_LEN_SETTING, "%s",
			driver_settings_get_boot_ret_str ( conf->boot_retry_count ) );
	HERMON_STORE_SETTING( origin, boot_retries_setting, buf );

	HERMON_STORE_STR_SETTING( origin, wol_setting, ( nic_conf->wol_conf.en_wol_magic ? STR_ENABLE : STR_DISABLE ) );

	return 0;
}

static int hermon_set_fw_settings ( struct hermon *hermon, struct firmware_image_props *props ) {
	struct settings *menu_settings = & ( hermon->hermon_settings.generic_settings.settings );
	struct settings *origin;
	char buf[HERMON_MAX_STR_LEN_SETTING];
	int rc;
	...
	if ( rc || ( !rc && !origin ) ) {
		DBGC ( hermon, "Failed to find FW image properties settings block\n" );
		return rc;
	}

	HERMON_STORE_STR_SETTING( origin, fw_version_setting, props->family_fw_version );
	HERMON_STORE_STR_SETTING( origin, flex_version_setting, props->flexboot_version );

	return 0;
}

static int hermon_set_hermon_settings ( struct hermon *hermon ) {
	struct settings *settings = & ( hermon->hermon_settings.generic_settings.settings );
	struct hermon_nvmem_conf *nv_conf = & ( hermon->hermon_nv_conf );
	struct device *dev = & ( hermon->pci->dev );
	char buf[HERMON_MAX_STR_LEN_SETTING];
	int rc;

	HERMON_STORE_STR_SETTING( settings, virt_mode_setting,
                        ( hermon->hermon_nv_conf.virt_conf.virt_mode ? STR_SRIOV : STR_NONE ) );
	HERMON_STORE_INT_SETTING( settings, virt_num_setting, nv_conf->virt_conf.num_of_vfs );
	HERMON_STORE_INT_SETTING( settings, virt_num_max_setting, nv_conf->max_num_of_vfs_supported );
	HERMON_STORE_STR_SETTING( settings, device_name_setting, nv_conf->device_name );
	HERMON_STORE_STR_SETTING( settings, chip_type_setting, dev->driver_name );
	HERMON_STORE_INT_SETTING( settings, pci_id_setting, dev->desc.device );
	HERMON_STORE_STR_SETTING( settings, bus_dev_fun_setting, dev->name );

	return 0;
}

static int hermon_set_general_settings ( struct hermon_port *port ) {
	struct settings *menu_settings = & ( port->driver_settings.generic_settings.settings );
	struct hermon *hermon = ib_get_drvdata( port->ibdev );
	struct hermon_nvmem_port_conf *port_nv_conf = & ( port->port_nv_conf );
	...
	HERMON_STORE_STR_SETTING( menu_settings, mac_add_setting, netdev_addr ( netdev ) );

	return rc;
}

static int hermon_register_nv_settings ( struct hermon *hermon ) {
	struct hermon_nvmem_conf *conf = & ( hermon->hermon_nv_conf );
	struct firmware_image_props *fw_image_props = &hermon->hermon_nv_conf.fw_image_props;
        struct hermon_tlv_header tlv;
        char *device_name;
        int rc = 0;
	...
	/* Set device name according to device ID */
        device_name = get_device_name ( hermon );
        strcpy ( conf->device_name, device_name);

        conf->max_num_of_vfs_supported = hermon->defaults.max_vfs;

	 /* Read virtualization configuration */
        memset ( &tlv, 0, sizeof ( tlv ) );
        tlv.type        = VIRTUALIZATION_TYPE;
        tlv.type_mod    = 0;
        tlv.length      = sizeof ( conf->virt_conf.dword );
        tlv.data        = ( void* ) & conf->virt_conf.dword;

        if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) ) {
                DBGC ( hermon, "Failed to read the virtualization configuration from the flash (rc = %d)\n", rc );
		conf->virt_conf.num_of_vfs      = hermon->defaults.total_vfs;
		conf->virt_conf.virt_mode       = hermon->defaults.sriov_en;
	}

	/* Set general settings */
	hermon_set_hermon_settings ( hermon );
	/* Read and set firmware settings */
	if ( ( rc = hermon_flash_read_fw_image_props ( hermon, fw_image_props ) ) )
		DBGC ( hermon, "Failed to read firmware image properties (rc = %d)\n", rc );
	if ( ( rc = hermon_set_fw_settings ( hermon, fw_image_props ) ) )
		DBGC ( hermon, "Failed to register FW settings\n" );

	return rc;
}

static int hermon_register_port_nv_settings ( struct hermon *hermon, unsigned int port_num ) {
	struct hermon_port *port = &hermon->port[port_num];
	struct hermon_nic_conf *nic_conf = &port->port_nv_conf.nic;
	struct hermon_iscsi_conf *iscsi_conf = &port->port_nv_conf.iscsi;
	int rc;

	if ( hermon->cap.nv_mem_access_supported == 0 )
		return 0;

	memset ( & ( port->port_nv_conf ), 0, sizeof ( port->port_nv_conf ) );

	/* Read NIC configuration from NV memory */
	if ( ( rc = hermon_flash_read_nic_boot_config ( hermon, &hermon->port[port_num] ) ) ) {
		DBGC ( hermon, "Failed to read NIC configuration from NV memory (rc = %d)\n", rc );
	}

	/* Read WOL configuration from NV memory */
        if ( ( rc = hermon_flash_read_nic_wol_config ( hermon, &hermon->port[port_num] ) ) ) {
                DBGC ( hermon, "Failed to read WOL configuration from NV memory (rc = %d)\n", rc );
        }

	/* Read iSCSI configuration from NV memory */
	if ( ( rc = hermon_flash_read_iscsi_config ( hermon, port_num, iscsi_conf ) ) ) {
		DBGC ( hermon, "Failed to read iSCSI configuration from NV memory (rc = %d)\n", rc );
	}

	/* Read general settings */
	if ( ( rc = hermon_flash_read_general_settings ( &hermon->port[port_num] ) ) ) {
		DBGC ( hermon, "Failed to read general settings from NV memory (rc = %d)\n", rc );
	}

	/* Register NIC configuration */
	if ( ( rc = hermon_set_nic_settings ( port, nic_conf ) ) ) {
		DBGC ( hermon, "Failed to register NIC settings\n" );
	}
	/* Register iSCSI configuration */
	if ( ( rc = hermon_set_iscsi_settings ( port, iscsi_conf ) ) ) {
		DBGC ( hermon, "Failed to register iSCSI settings\n" );
	}
	/* Register general configuration */
	if ( ( rc = hermon_set_general_settings ( port ) ) ) {
		DBGC ( hermon, "Failed to register general settings\n" );
	}

	return 0;
}

int hermon_port_get_defaults ( struct hermon *hermon, unsigned int port_num ) {
...
int hermon_get_ini_and_defaults ( struct hermon *hermon ) {
	struct hermonprm_query_romini ini;
	union hermonprm_query_defparams def;
	int rc, i;
...
int hermon_get_online_flash_option_rom_en ( struct hermon *hermon,
	...
	struct hermon_tlv_header tlv;
	union hermon_nic_boot_conf conf;
        int rc;
        u8  option_rom_en;
	...
	if ( ( rc = hermon_read_conf_tlv ( hermon, &tlv ) ) ) {
		DBGC ( hermon, "Failed to read the boot TLV from the flash (rc = %d)\n", rc );
		/* This failure could be due to missing TLV - return the default value */
		option_rom_en		= hermon->port[port - 1].defaults.boot_option_rom_en;
		legacy_boot_protocol	= hermon->port[port - 1].defaults.boot_protocol;
	} else {
		...
		for ( vep = 0; vep < supported_veps[i]; vep ++ ) {
			...
			if ( boot_post_shell ) {
				boot_enable = 1;
	                        ...
	                        if ( ( rc = hermon_get_online_flash_option_rom_en ( hermon,
						i + 1, &boot_enable ) ) != 0 )
					return rc;
			}
			/*
...
static void hermon_updater ( void *priv, uint8_t status ) {
	...
	switch ( status ) {
		...
		case STATUS_UPDATE_URI_BOOT:
			...
			break;
		default:
			DBGC ( hermon, "Update failed due to unknown status. Aborting...\n" );;
			return;
	}

	hermon_set_ocbb_event_value ( hermon, 36, val );
};
...
static void copy_netdev_settings_to_netdev ( struct net_device *src, struct net_device *dest ) {
...
static int hermon_probe ( struct pci_device *pci )
{
	...
	for ( i = 0 ; i < hermon->num_ports ; i++ ) {
		...
		if ( hermon->cap.nv_mem_access_supported ) {
			...
			hermon_register_port_nv_settings ( hermon, i );
		}
	...
	if ( hermon->cap.nv_mem_access_supported )
		hermon_register_nv_settings ( hermon );
